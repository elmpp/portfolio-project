/**
 * A button but spring animated and styled
 *
...
import {styleVariables, Theme, useTheme, styled} from '../../../styles'
import colorUtil from 'color'
import {space, layout, typography, color, flex, compose} from 'styled-system'
// import {white, theme.colors.black} from '../../../styles/color'
...
const ButtonComponent: ReactFCSpecifyChildren<ButtonProps> = React.memo(props => {
  const {
  ...
  console.log('style', util.inspect(style, {showHidden: false, depth: undefined, colors: true}))
  ...
  const getStyles = useCallback(
    ({forceImmediate}: {forceImmediate: boolean}) => {
      ...
      const hoverBackgroundColor =
        ...
        theme.util.opacity(4, backgroundColor)
      ...
      const foregroundColor =
        (disabled && theme.util.opacity(4)) ||
        (transparent && (buttonColor || theme.colors.primary)) ||
        (!transparent && theme.util.opacity(0))
      // const foregroundColor =
      //   (disabled &&
      //     colorUtil(theme.dark ? theme.colors.white : theme.colors.black)
      //       .alpha(0.32)
      //       .rgb()
      //       .string()) ||
      //   (transparent && (buttonColor || theme.colors.primary)) ||
      //   (!transparent && theme.dark ? theme.colors.white : colorUtil(backgroundColor).isDark() ? theme.colors.white : theme.colors.black)
      ...
      const hoverForegroundColor = theme.util.opacity(0, hoverBackgroundColor)
      // const hoverForegroundColor = colorUtil(colorUtil(hoverBackgroundColor).isDark ? theme.colors.white : foregroundColor)
      //   .alpha(0.6)
      //   .rgb()
      //   .string()
            ...
            ? hoverBackgroundColor ||
              (transparent
                ...
                : colorUtil(textColor)
                    ...
                    .rgb()
                    .string())
            : colorUtil(transparent ? hoverBackgroundColor || backgroundColor : backgroundColor)
                .alpha(0)
                .rgb()
                .string(),
        textColor,
